/**
 * Using Rails-like standard naming convention for endpoints.  --
 * GET     /api/products           ->  index
 * GET     /api/products/:id       ->  show
 * POST    /api/products           ->  create
 *
 * Uso de la convención de nomenclatura estándar similar a Rails para puntos finales. -
 * GET / api / products -> index --  indice
 * GET / api / products /: id -> show --  mostar
 * POST / api / productos -> create -- crear
 *
 */

const Product = require('./product.model');


//funciones que no ayudan a la legibilidad al responder a las promesas

function respondWithResult(res, code) {
    const statusCode = code || 200;
    return (entity) => {
        if (entity) {
            res.status(statusCode).json(entity);
        }
    };
}

function handleEntityNotFound(res) {
    return (entity) => {
        if (!entity) {
            res.status(404).end();
            return null;
        }
        return entity;
    };
}

function hadleError(res, code) {

    const statusCode = code || 500;
    return (err) => {
        res.status(statusCode).send(err);
    };
}


//Gets a list products  --  muestra la lista deproductos
function index(req, res) {
    return Product.find().exec()
        .then(respondWithResult(res))
        .catch(hadleError(res));
}

//Create products - -  crea productos
function create(req, res) {
    return Product.create(req.body)
        .then(respondWithResult(res, 201))
        .catch(hadleError(res));

}

// Gets a single product from the DB --  muestra un solo producto sobre la base de datos
function show(req, res) {
    return Product.findById(req.params.id).exec()
        .then(handleEntityNotFound(res))
        .then(respondWithResult(res))
        .catch(hadleError(res));
}

function remove(req, res) {
    return Product.findByIdAndRemove(req.params.id).exec()
        .then(handleEntityNotFound(res))
        .then(respondWithResult(res))
        .catch(hadleError(res));
    console.log("remove");

}

function update(req, res) {

    return Product.findByIdAndUpdate(req.params.id, req.body).exec()
        .then(handleEntityNotFound(res))
        .then(respondWithResult(res.body("update")))
        .catch(hadleError(res));
    console.log("update");
}


module.exports = {
    index,
    create,
    show,
    remove,
    update
};